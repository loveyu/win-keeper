...
#[test]
fn dropping_handle_shuts_down_service() {
    use ksni::blocking::TrayMethods as _;

    let connection = session_connection();
    let watcher = WatcherHandle::start(true).unwrap();
    let (tray, _) = TestTray::<false>::new("drop-shutdown-tray");
    ...
    wait_until(
        DEFAULT_TIMEOUT,
        || !has_owner(&connection, &service_name),
        "tray service shutdown after dropping its last handle",
    );
    watcher.close();
}
